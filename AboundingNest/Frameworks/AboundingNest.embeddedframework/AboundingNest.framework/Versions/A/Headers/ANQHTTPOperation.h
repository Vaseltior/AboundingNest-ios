...
/**
    ANQHTTPOperation is a general purpose NSOperation that runs an HTTP request. 
    ...
    op = [[[ANQHTTPOperation alloc] initWithURL:url] autorelease];
    ...
    As mentioned above, ANQHTTPOperation is very general purpose.  There are a 
...
@interface ANQHTTPOperation (NSURLConnectionDelegate)
...
- (BOOL)httpOperation:(ANQHTTPOperation *)operation canAuthenticateAgainstProtectionSpace:(NSURLProtectionSpace *)protectionSpace;
- (void)httpOperation:(ANQHTTPOperation *)operation didReceiveAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge;
